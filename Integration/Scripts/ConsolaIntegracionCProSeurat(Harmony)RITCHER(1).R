...
setwd("D:/EVOSdata/Ceci/Desarrollo2025/CPro(2)/CCA/IntegracionHarmonyX6")
getwd()
...
OBJ05 <- readRDS(file = "CProSeuratOBJNEG_05.rds")
OBJ05$Patient <- "05_PacienteRT1"

OBJ06 <- readRDS(file = "CProSeuratOBJNEG_06.rds")
OBJ06$Patient <- "06_PacienteRT2"


# Create a list of Seurat objects
obj.list <- list(OBJ00, OBJ01, OBJ02, OBJ04, OBJ05, OBJ06)
...
seurat.merged <- merge(obj.list[[1]], 
                       ...
                       add.cell.ids = c("Pac1", "Pac2", "Pac3", "Pac4", "Pac5", "Pac6"), 
                       project = "CLL_Morpho")
...
DimPlot(seurat.merged, group.by = "Patient", label=TRUE) # This will show batch effects
DimPlot(seurat.merged, split.by = "Patient") # This will show batch effects
DimPlot(seurat.merged, split.by = "Patient", label=TRUE) # This will show batch effects
...
saveRDS(seurat.integrated.harmony, "seurat_integrated_harmonyX6RTFull.rds")
...
DimPlot(seurat.integrated.harmony, group.by = "Patient", pt.size = 2) # 
DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters", pt.size = 2, label=TRUE) # 
DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters", pt.size = 2) # 

plot <- DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters", pt.size = 2) # 

plot_repelled_clusters <- LabelClusters(plot = plot, id = "seurat_clusters", repel = TRUE)
print(plot_repelled_clusters)

DimPlot(seurat.integrated.harmony, split.by  = "Patient", pt.size = 2, label=TRUE)) # 



DimPlot(seurat.integrated.harmony, split.by = "Patient", label=TRUE) # This will show batch effects
DimPlot(seurat.integrated.harmony, split.by = "Patient", pt.size = 2) # This will show batch effects



DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters",split.by = "Patient", label=TRUE, pt.size = 2)
plot <- DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters",split.by = "Patient", pt.size = 2)
plot_repelled_clusters <- LabelClusters(plot = plot, id = "seurat_clusters", repel = TRUE)
print(plot_repelled_clusters)
...
DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters",split.by = "Patient", pt.size = 2)
DimPlot(seurat.integrated.harmony, group.by = "seurat_clusters", label=TRUE)
DimPlot(seurat.integrated.harmony, group.by = "RNA_snn_res.1", split.by = "Patient", label=TRUE)


str(seurat.integrated.harmony)

view(seurat.integrated.harmony@meta.data)
colnames(seurat.integrated.harmony)
...
write.csv( Proportions, "ProportionsHarmonyRTfull.csv")

metadata <- seurat.integrated.harmony@meta.data
Proportions <- table(metadata[,c("Patient","RNA_snn_res.0.5")])
Proportions <- table(metadata[,c("Patient","RNA_snn_res.0.5")])

write.csv( Proportions, "ProportionsHarmonyRES05.csv")
